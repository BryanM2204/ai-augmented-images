...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("/home/bam20007/ai-augmented-images/checkpoints/model_v3_checkpoint/best_vit_model.pth", weights_only=True))
    model.to(device)

    # 2. Load Data
    data_dir = os.path.expanduser('~/ai-augmented-images/data/final_dataset_v2')
    _, _, test_loader = get_dataloaders(data_dir=data_dir, batch_size=16)
    ...
    for bias in bias_values:
        ...
        ai_gen_recall = report['AI-Generated']['recall']
        macro_f1 = report['macro avg']['f1-score']
        
        results.append((bias, auth_recall, fake_recall, ai_gen_recall, macro_f1))
        print(f"{bias:<10} | {auth_recall:<12.2f} | {fake_recall:<12.2f} | {ai_gen_recall:<12.2f} | {macro_f1:<10.2f}")
