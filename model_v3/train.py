...
def save_checkpoint(model, optimizer, scheduler, history, epoch, step, filename="vit_base_16_384/checkpoint.pth"):
    checkpoint = {
        'epoch': epoch,
        'step': step,
    ...
    pd.DataFrame(history).to_csv("vit_base_16_384/training_log.csv", index=False)

def load_checkpoint(model, optimizer, scheduler, filename="vit_base_16_384/checkpoint.pth"):
...
def train_model(epochs=10, batch_size=16, learning_rate=2e-5):
    ...
    os.makedirs("vit_base_16_384", exist_ok=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            torch.save(model.state_dict(), "vit_base_16_384/best_vit_model.pth")
